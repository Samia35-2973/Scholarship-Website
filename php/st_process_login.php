...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $usermail = clean_data($_POST['user_mail']);
    ...
    if (count($errors) > 0) {
        ...
        $select = "SELECT * FROM students WHERE email = '$usermail' && password = '$pass_word'";
        $result = mysqli_query($conn, $select);
        if (mysqli_num_rows($result) > 0) {
            $query  = "SELECT last_name FROM students WHERE email = '$usermail' && password = '$pass_word'";
